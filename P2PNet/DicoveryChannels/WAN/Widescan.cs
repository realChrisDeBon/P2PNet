...
namespace P2PNet.DicoveryChannels.WAN
    {
        ...
        public class Widescan
            {
            private static volatile ConcurrentQueue<string> ipv6Queue = new ConcurrentQueue<string>();
            ...
            public static MaximumMemoryAllocation MaximumMemoryAllocated
                ...
                set
                    {
                    if (MemoryAllocations.ContainsKey(maximummemoryallowed))
                        {
                        // Turn on memory cap
                        HasMemoryLimit = true;
            ...
            public static async void StartWidescan()
                ...
                Thread reader = new Thread(ReadAddressesFromPipe);
                switch (HardwareMode)
                    {
                    ...
                    case HardwareMode.Accelerator:
                        ...
                        if (HasMemoryLimit == true)
                            {
                            Task.Run(() => AcceleratorWorker(true));
                            }
                        else
                            {
                            Task.Run(() => AcceleratorWorker());
                            }
                        break;
            ...
            private static void ReadAddressesFromPipe()
                {
                using (NamedPipeClientStream pipeClient = new NamedPipeClientStream(".", "IPv6Pipe", PipeDirection.In))
                    {
                    pipeClient.Connect();
                    using (StreamReader reader = new StreamReader(pipeClient))
                        {
                        while (true) // implement cancellation token 
                            {
                            string ipv6Address = reader.ReadLine();
                            if (ipv6Address == null)
                                break;

                            ipv6Queue.Enqueue(ipv6Address);
                            }
                        }
                    }
                }
            ...
            static async Task GPUWorker()
                {
                using (NamedPipeServerStream pipeServer = new NamedPipeServerStream("IPv6Pipe", PipeDirection.Out, 1, PipeTransmissionMode.Byte))
                    {
                    // 1. Initialize OpenCL
                    ErrorCode error;
                    ...
                    using (StreamWriter writer = new StreamWriter(pipeServer))
                        ...
                        for (long chunkOffset = 0; chunkOffset < numAddresses; chunkOffset += chunkSize)
                            {
                            int x = GC.GetGeneration(ipv6Queue);
                            GC.Collect(x);
            ...
            static async Task GPUWorker(bool limit)
                {
                using (NamedPipeServerStream pipeServer = new NamedPipeServerStream("IPv6Pipe", PipeDirection.Out, 1, PipeTransmissionMode.Byte))
                    {
                    // 1. Initialize OpenCL
                ...
                foreach (string addressPrefix in addressPrefixes)
                    ...
                    Parallel.For(0, numberOfTasks, taskIndex =>
                    {
                        ...
                        for (BigInteger address = taskStart; address <= taskEnd; address++)
                            {
                            ipv6Queue.Enqueue(address.ToString());
                            }
                    });
                    }
                }
            static async Task CPUWorker(bool limit)
                {
                foreach (string addressPrefix in addressPrefixes)
                    {
                    string[] segments = addressPrefix.Split(':');
            ...
            static async Task AcceleratorWorker()
                {

                }
            static async Task AcceleratorWorker(bool limit)
                {
            ...
            private static class PacketIntercept
                ...
                internal const string space_buffer = "     ";
                ...
                private static void LogPacketDetails(IPv6Packet ipPacket, IcmpV6Packet icmpPacket)
                    {
#if DEBUG
                    // Here you could log details to a file or database
            ...
            public static async Task SendOutboundPing()
                {

                while (true) // Implement cancellation token
                    {

                    ipv6Queue.TryDequeue(out string ip_last4_segments);
                    ...
                    try
                        ...
                        // Create raw data buffer for ICMPv6 packet
                        byte[] icmpData = Encoding.UTF8.GetBytes(messagepayload);
                        // Array.Copy(Encoding.ASCII.GetBytes($"Listening:{ConnectionHandler.listenerPort}"), icmpData, 14);
                        ...
                        var ipPacket = new IPv6Packet(
                            System.Net.IPAddress.Parse(PeerNetwork.PublicIPV6Address.ToString()), // Source IP Address
                        ...
                        foreach (var device in PacketIntercept.devices)
                            ...
                            try
                                ...
                                if (error == false)
                                    {
                                    // No error going out, so we send two more
                                    device.SendPacket(ethernetPacket);
                                    Thread.Sleep(50);
                                    device.SendPacket(ethernetPacket);
                                    }
